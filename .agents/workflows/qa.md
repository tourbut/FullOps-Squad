...
- 대상 코드가 `main` 브랜치/스테이징에 빌드되어 있어야 함.

## Steps
1. **Common Preamble** 수행.
2. 테스트 목록·엣지 케이스를 도출하고 자동/수동 테스트를 실행한다 (요청 범위 밖 기능 추가 검증 금지).
   - 브라우저로 검증해야 하는 사용자 플로우(로그인, 폼 제출, 멀티스텝 시나리오 등)는 `webwright` 스킬로 수행한다: `plan.md`에 critical point를 정의 → Playwright(Firefox) 스크립트로 실행 → 스크린샷·로그로 각 CP를 시각 검증 → `sandbox/.venv/bin/python` 인터프리터 사용. 산출 증거(스크린샷/`final_runs/`)는 QA 리포트에 인용한다.
   - 단순 셀렉터 기반 E2E는 `webapp-testing` 스킬을 사용한다.
